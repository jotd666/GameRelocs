...
for i,line in enumerate(af.lines):
    if line.strip().strip(":") in al:
        addr = []
        for x in (af.lines[i+1:i+3]):
            toks = x.split()
            if toks:
                if toks[0].lower() == "dc.w":
                    addr.append(toks[1])
                else:
                    addr.append(x.split(":")[1].strip()[:4])
        print(line.strip(),addr,i)
        if "fff" in addr[0]:
            pass
        else:
            address = int(addr[1],16)
            af.lines[i] += "\tdc.l\tlb_{:x}\t;%autoins%\n".format(address)
